...
class DBHelper (context: Context, factory: SQLiteDatabase.CursorFactory?) :
    ...
    fun updateTeamWithBillDetails(food_name : String ,liquor_name: String, contriForEachMember : Double,  member_name:String ) {
        // below we are creating
        // a content values variable
        val db = this.writableDatabase
    ...
    fun updateBillWithTeamName(food_name : String ,liquorName : String, member_name:String ) {
        ...
        if (cursor != null && cursor.count > 0) {
            ...
            if(food_name != null){
                ...
                sqlQuery =  "select team_members_Ids From Bill where liquor_name = '" + liquorName + "'"
            }
            ...
            if (cursor1 != null && cursor1.count > 0) {
                ...
                // appending value in the text view
                cursor1!!.moveToFirst()
                var old_TeamMember_Id = cursor1.getString(cursor1.getColumnIndex(DBHelper.Team_Members_IDs))

                if (TeamMember_Id == "") {

                    while (cursor1.moveToNext()) {
                        old_TeamMember_Id = cursor1.getString(cursor1.getColumnIndex(DBHelper.Team_Members_IDs))
                    }
                }
